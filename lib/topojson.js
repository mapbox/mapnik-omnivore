...
TopoJSON.prototype.getLayers = function(callback) {
  ...
  if (!featureset) return callback(invalid('Invalid topojson: TopoJSON file must be a FeatureCollection'));
  if (featureset.next() === undefined) return callback(invalid('Invalid topojson: does not contain any features'));
