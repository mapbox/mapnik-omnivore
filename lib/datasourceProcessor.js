...
function processRasterDatasource(file, filesize, name, info, filetype, callback) {
    ...
    try{
        ...
        getMinMaxZoomGDAL(info.raster, results.center, info.proj4, function(err, min, max) {
            if (err) return callback(err);
            //Values to input into xml for Mapnik
            return callback(null, {
                extent: results.extent,
...
// using center lat
function getMinMaxZoomGDAL(info, center, proj, callback){
    


    //Y = lat (horizontal)
    //X = lon (vertical)
    //Transform pixels/unit
    var halfPixelsPerUnit = info.units.linear.value/2;
    console.log(halfPixelsPerUnit);
    var refFrom = gdal.SpatialReference.fromProj4(proj);
    var refTo = gdal.SpatialReference.fromProj4('+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs');
    var transform = new gdal.CoordinateTransformation(refFrom, refTo);
    // console.log(center);
    // console.log(center[1] - halfPixelsPerUnit);
    console.log(center[0] + "," + (center[1] - halfPixelsPerUnit));
    ...
    line.points.add(center[0], (center[1] + halfPixelsPerUnit));
    console.log("length before: "+line.getLength());
    line.transform(transform);
    console.log("length after: "+line.getLength());
    var mercPixelSize = line.getLength();


    // var lat = center[1];
    // console.log(info.pixelSize[0]);
    // console.log(info.pixelSize[1]);
    // var lonDiff = center[0] + info.pixelSize[0];
    // var latDiff = center[1] + (-info.pixelSize[1]);
    // console.log("center lat: " + center[1]);
    // console.log("center lon: " + center[0]);
    // console.log("diff lon: " + lonDiff);
    // console.log("diff lat: " + latDiff);

    //if projection is spherical mercator
    // if(proj.indexOf('WGS84') !== -1){

    //     //get distance using spherical
    //     var distance = spherical.distance(center, [lonDiff, latDiff]);
    //     console.log("distance as WGS84: " + distance);
    // }

    // var pointCenter = new gdal.Point(center[0], center[1]);
    // var pointPixelDiff = new gdal.Point(lonDiff, latDiff);

    // var distance = pointCenter.distance(pointPixelDiff);
    ...
    for(var zoom = 19; zoom >=0; zoom--){
        res = circ * Math.cos(center[0] * (Math.PI/180)) / Math.pow(2,(zoom+8));
        console.log(res);
        console.log(mercPixelSize);
        //use the file's native resolution as the threshold
        if(res >= mercPixelSize){
            maxzoom = zoom;
...
function projectionFromRaster(file, callback) {
    ...
    try {
        ...
        console.log(linearUnits);
        var angularUnits = ref.getAngularUnits();
        console.log(angularUnits);
