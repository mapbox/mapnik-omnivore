...
function processPostgisDatasource(params, filesize, name, proj, filetype, callback) {
    ...
    var ds;
    try { ds = new mapnik.Datasource(mapnikOptions); }
    catch(err) { return callback(invalid(err)); }
...
function projectionFromPostgis(params, callback) {
    ...
    // Make sure a table is in some way defined
    var table = params.geometry_table || params.table;
    if (!table) return callback(invalid('No table defined in postgis parameters'));

    // If a SQL query is specified without `geometry_table` then we're in an ambiguous situation
    var isQuery = table.split(' ').length > 1;
    if (isQuery) return callback(invalid('Ambiguous geometry information in postgis parameters'));

    // Queries geometry_columns for SRS, defaulting to the first column if more than one
    var querystring = "SELECT get_proj4_from_srid((";
    querystring += "SELECT srid from geometry_columns where f_table_name = '" + table;
    if (params.geometry_field) querystring += "' AND f_geometry_column = '" + params.geometry_field;
    querystring += "' LIMIT 1)) as srs;";
    ...
    client.connect(function(err) {
        ...
        client.query(querystring, function(err, result) {
            if (err) return callback(err);
            if (!result.rows[0].srs)
                return callback(invalid('Postgis geometry column could not be indentified'));
            callback(null, result.rows[0].srs.trim());
        });
    });
}
/************
