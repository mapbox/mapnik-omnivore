...
function getProjection(file, filetype, callback) {
    if (filetype === '.geo.json' || filetype === '.csv' || filetype === '.topojson') return callback(null, '+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs');
    //else if shape
...
function getCenterAndExtent(ds, projection, filetype) {
    ...
    } else if (filetype === '.kml' || filetype === '.geo.json' || filetype === '.gpx' || filetype === '.topojson') {
        try {
            extent = ds.extent();
        } catch (err) {
...
function getDatasource(file, name, filetype, callback) {
    ...
    if (filetype === '.kml') {
        ...
        //Get layer for .geo.json/topojson
    } else if (filetype === '.geo.json' || filetype === '.topojson') {
        var options = {
            type: dstype,
            file: file,
            layer_by_index: 0
