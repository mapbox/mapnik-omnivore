    ...
    getProjection(file, filetype, function(err, proj) {
        ...
        // else OGR datasource
            ...
            processOgrDatasource(file, filesize, proj, filetype, function(err, info) {
                console.log("about to return callback");
                if (err) return callback(err);
                return callback(null, {
                    filesize: filesize,
                    projection: proj,
...
function processOgrDatasource(file, filesize, proj, filetype, callback) {
    console.log("in processOgrDatasource");
...
function getDatasourceProperties(file, filetype, callback) {
    ...
    if (filetype === '.kml') {
        ...
        getKmlLayers(options, function(err, layers) {
            if (err) return callback(err);
            getDatasource(options, layers, function(err, json, ds) {
                if (err) return callback(err);
...
function getDatasource(options, layers, callback) {
    ...
    if (options.layer_by_index === 0) {
        ...
        layers.forEach(function(layer) {
            ...
            try {
                var ds = new mapnik.Datasource(options);
                //Grabs the first datasource that is not empty, and later used to obtain center and extent
                if (ds.extent().indexOf(0) === -1){
                    validDatasource = ds;
                    return callback(null, ds);
                }
            } catch (err) {
                return callbac(invalid('Error creating Mapnik Datasource for file: ' + err.message));
            }
        });
    }
};
