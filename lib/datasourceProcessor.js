...
function getDatasource(options, layers, callback) {
    ...
    if (options.layer_by_index === 0) {
        ...
        if (validDatasource !== undefined && error === undefined) return callback(null, validDatasource);
        else if (error === undefined) return callback(invalid('Invalid extent in datasource, probably [0,0,0,0]'));
