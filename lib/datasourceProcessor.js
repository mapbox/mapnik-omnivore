...
function getMinMaxZoomGDAL(info, lat, callback){
    ...
    var res;
    var circ;
    // check which unit for which circumference
    if(info.unit === 'meters'){
        circ = 40075000;
    }
    ...
    for(var zoom = 0; zoom <= 19; zoom++){
        res = circ * Math.cos(lat * (Math.PI/180)) / (2 ^ zoom);
        console.log("res: " + res);
        //determine a good threshold
...
function projectionFromRaster(file, callback) {
    ...
    try {
        ...
        return callback(invalid('Invalid .TIFF file.' + err));
    }   

    //TODO: check if the current projection is something we can deal with
    //...what are some projections we can't deal with?
    if(srsResult.proj4 === undefined){
        return callback(invalid('Invalid .TIFF file.'));
