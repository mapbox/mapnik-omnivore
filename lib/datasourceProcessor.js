...
function getMinMaxZoomGDAL(raster, center, proj, callback){
    //Y = lat (horizontal)
    //X = lon (vertical)

    //Should I be using the linearunits here?
    var halfPixelsPerUnit = raster.units.linear.value/2;
    
    //Or should I be using the pixelsize that comes from geotransform()?
    //var halfPixelsPerUnit = raster.pixelSize[0]/2;
    ...
    for(var zoom = 19; zoom >=0; zoom--){
        //There seem to be a number of different formulas to use in order to calculate resolution (meters/pixel). 
        //Should I be calculating "ground resolution"? That equation is provided here: http://msdn.microsoft.com/en-us/library/bb259689.aspx
        //and looks like this:
        //map width = map height = 256 * 2 level pixels
        //ground resolution = cos(latitude * pi/180) * earth circumference / map width

        //This equation calculates meters/pixel based on a particular latitude (should latitude be in radians or degrees?)
        //res = circ * Math.cos(center[1] * (Math.PI/180)) / Math.pow(2,(zoom+8));
        
        //slightly different forumla from http://wiki.openstreetmap.org/wiki/Zoom_levels
        //S=C*cos(y)/2^(z+8)
        res = circ * Math.cos(center[1]) / Math.pow(2,(zoom+8));
        console.log(res);
        console.log(mercatorPixelSize);
