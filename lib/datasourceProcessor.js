...
var ogr = gdal.ogr;
//require('gdal').SpatialReference.fromUserInput(<string>;).getLinearUnits()
...
function processRasterDatasource(file, filesize, name, info, filetype, callback) {
    ...
    var ds = new mapnik.Datasource(options);
    var ref = ogr.SpatialReference.fromProj4(info.proj4);
    var results = getCenterAndExtent(ds, info.proj4, filetype);
    getMinMaxZoomGDAL(info, results.center[1], function(err, min, max) {
        if (err) return callback(err);
        //Values to input into xml for Mapnik
        return callback(null, {
            extent: results.extent,
...
function getMinMaxZoomGDAL(info, lat, callback){
    ...
    if(info.unit === 'meters') circ = 40075000;
    // else if(info.unit === 'feet') circ = 131479658.79;
    // else if(info.unit === 'miles') circ = 24901.450529;
    // else if(info.unit === 'cm') circ = 4007500000;
    // else if(info.unit) === 'km') circ = 40075;
    // else if(info.unit) === 'yards') circ = 43826552.931;
    // else if(info.unit) === 'inches') circ = 1577755905.5; 
...
function projectionFromRaster(file, callback) {
    ...
    var geotransform;
    var bands =[];
    //examples: https://github.com/naturalatlas/node-gdal/blob/d1b7d9039874da9db8f70ca0ac8dcef878ede694/examples/rasterinfo.js
    try {
        ds = gdal.open(file);
        ...
        for(var i = 1; i <= bandCount; i++){
            ...
            bands.push({'band':band, 'stats':bandStats, 'nodata':band.getNoDataValue()});
        };
        console.log(bands);
    ...
    if(srsResult.proj4 === undefined){
        return callback(invalid('Invalid .TIF file.'));
