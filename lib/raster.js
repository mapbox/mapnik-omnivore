...
function Raster(filepath) {
  ...
  try {
    ...
    throw invalid('Invalid raster: could not open the file', err);
  }
  ...
  try {
    ...
    throw invalid('Invalid raster: could not read image dimensions', err);
  }
  ...
  try {
    ...
    throw invalid('Invalid raster: could not read georeferencing information', err);
  }

  try { this.projection = srs.parse(this.gdalDatasource.srs.toWKT()).proj4; }
  catch (err) {
    throw invalid('Invalid raster: could not read spatial reference information', err);
  }
}
...
Raster.prototype.getBands = function() {
  ...
  for (i = numBands; i <= numBands; i++) {
    ...
    try {
      ...
      throw invalid('Invalid raster: could not read band information', err);
    }
