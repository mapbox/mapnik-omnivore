...
function Raster(filepath) {
  ...
  var abs_geo1 = Math.abs(geotransform[1]);
  var abs_geo5 = Math.abs(geotransform[5]);
  var totals_abs = Math.abs(abs_geo1 / abs_geo5);

  if (geotransform[1] <= 0 || geotransform[5] >= 0) throw invalid('Invalid raster: Invalid pixelsize in geotransform array');
  if ((totals_abs - 1) > 1) throw invalid('Invalid raster: Invalid pixelsize in geotransform array');
