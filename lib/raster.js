...
function Raster(filepath) {
  ...
  // Validate geotransform array values. This could negatively affect extent values later on.
  if (geotransform[1] <= 0 || geotransform[5] >= 0) throw invalid('Invalid raster: Invalid pixelsize in geotransform array');
  if (Math.round(1000 * (Math.abs(geotransform[5]))) / 1000 !== Math.round(1000 * geotransform[1]) / 1000) throw invalid('Invalid raster: Invalid pixelsize in geotransform array');
